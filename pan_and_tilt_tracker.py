...
def main():
    ...
    pwmFrequency = 200                 # frequency in Hz
    pwmInitialDutyCycle = 90           # initial duty cycle in %
    ...
    while cv2.waitKey(1) != 27 and capWebcam.isOpened():                # until the Esc key is pressed or webcam connection is lost
        ...
        # end if
        imgOriginal2 = cv2.medianBlur(imgOriginal,5)
        imgHSV = cv2.cvtColor(imgOriginal2, cv2.COLOR_BGR2HSV)

      #  imgHSV = cv2.cvtColor(imgOriginal, cv2.COLOR_BGR2HSV)

        imgThreshLow = cv2.inRange(imgHSV, np.array([23, 97, 51]), np.array([35, 221, 255]))
        imgThreshHigh = cv2.inRange(imgHSV, np.array([111, 256, 100]), np.array([35, 221, 255]))
        ...
        if circles is not None:                     # this line is necessary to keep program from crashing on next line if no circles were found
            ...
            print "intXFrameCenter = "+ str(intXFrameCenter) + " panServoPosition = " + str(panServoPosition)
            print "intYFrameCenter = "+ str(intYFrameCenter) + " tiltServoPosition = " + str(tiltServoPosition)
            if x < intXFrameCenter and panServoPosition >= 5:
                panServoPosition = panServoPosition + 5
            elif x > intXFrameCenter and panServoPosition <= 278:
                panServoPosition = panServoPosition - 2
            # end if else

            if y < intYFrameCenter and tiltServoPosition >= 62:
                tiltServoPosition = tiltServoPosition - 5
            elif y > intYFrameCenter and tiltServoPosition <= 135:
                tiltServoPosition = tiltServoPosition + 5
            # end if else
